...
var $author$project$Main$Event = F5(
	function (info, from_time, to_time, flag, location) {
		return {flag: flag, from_time: from_time, info: info, location: location, to_time: to_time};
	});
var $elm$json$Json$Decode$bool = _Json_decodeBool;
var $elm$json$Json$Decode$map5 = _Json_map5;
var $author$project$Main$itemsDecoder = $elm$json$Json$Decode$list(
	A6(
		$elm$json$Json$Decode$map5,
		$author$project$Main$Event,
		A2($elm$json$Json$Decode$field, 'info', $elm$json$Json$Decode$string),
		A2($elm$json$Json$Decode$field, 'from_time', $elm$json$Json$Decode$string),
		A2($elm$json$Json$Decode$field, 'to_time', $elm$json$Json$Decode$string),
...
var $author$project$Main$dayScheduleDecoder = A3(
	...
	A2($elm$json$Json$Decode$field, 'events', $author$project$Main$itemsDecoder));
var $author$project$Main$scheduleDecoder = $elm$json$Json$Decode$list($author$project$Main$dayScheduleDecoder);
var $author$project$Main$schedule_url = '/schedule';
var $author$project$Main$getRequest = $elm$http$Http$get(
	{
		expect: A2($elm$http$Http$expectJson, $author$project$Main$GotEvents, $author$project$Main$scheduleDecoder),
		url: $author$project$Main$schedule_url
	});
var $author$project$Main$init = function (_v0) {
	return _Utils_Tuple2($author$project$Main$LoadingModel, $author$project$Main$getRequest);
};
...
var $author$project$Main$ErrorModel = function (a) {
	return {$: 'ErrorModel', a: a};
};
var $author$project$Main$EventModelNoDate = function (a) {
	return {$: 'EventModelNoDate', a: a};
};
var $author$project$Main$EventModelWithDate = F2(
	function (a, b) {
		return {$: 'EventModelWithDate', a: a, b: b};
	});
var $author$project$Main$errorToString = function (error) {
	switch (error.$) {
		case 'BadBody':
			var string = error.a;
			return 'Did not understand response from Loyola or schedule loader: ' + string;
		case 'NetworkError':
...
var $PanagiotisGeorgiadis$elm_datetime$Calendar$Internal$fromZonedPosix = F2(
	function (zone, posix) {
		return $PanagiotisGeorgiadis$elm_datetime$Calendar$Internal$Date(
			{
				day: $PanagiotisGeorgiadis$elm_datetime$Calendar$Internal$Day(
...
var $PanagiotisGeorgiadis$elm_datetime$Clock$Internal$fromZonedPosix = F2(
	function (zone, posix) {
		return $PanagiotisGeorgiadis$elm_datetime$Clock$Internal$Time(
			{
				hours: $PanagiotisGeorgiadis$elm_datetime$Clock$Internal$Hour(
...
var $PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$fromZonedPosix = F2(
	function (zone, posix) {
		return $PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$DateTime(
			{
				date: A2($PanagiotisGeorgiadis$elm_datetime$Calendar$Internal$fromZonedPosix, zone, posix),
				time: A2($PanagiotisGeorgiadis$elm_datetime$Clock$Internal$fromZonedPosix, zone, posix)
			});
	});
...
var $PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$getTimezoneOffset = F2(
	function (zone, posix) {
		var _v0 = _Utils_Tuple2(
			$PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$fromPosix(posix),
			A2($PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$fromZonedPosix, zone, posix));
		var dateTime = _v0.a;
		var zonedDateTime = _v0.b;
		return $PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$toMillis(zonedDateTime) - $PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$toMillis(dateTime);
	});
var $PanagiotisGeorgiadis$elm_datetime$DateTime$getTimezoneOffset = $PanagiotisGeorgiadis$elm_datetime$DateTime$Internal$getTimezoneOffset;
var $elm$time$Time$Posix = function (a) {
	return {$: 'Posix', a: a};
};
var $elm$time$Time$millisToPosix = $elm$time$Time$Posix;
var $author$project$Utils$localDateTime = F2(
	function (posix, zone) {
		var offset = A2($PanagiotisGeorgiadis$elm_datetime$DateTime$getTimezoneOffset, zone, posix);
		return $PanagiotisGeorgiadis$elm_datetime$DateTime$fromPosix(
			$elm$time$Time$millisToPosix(
				$elm$time$Time$posixToMillis(posix) + offset));
	});
var $author$project$Utils$PosixZone = F2(
	function (posix, zone) {
		return {posix: posix, zone: zone};
	});
...
var $author$project$Main$refreshDateTask = A2(
	$elm$core$Task$perform,
	$author$project$Main$ReceiveDate,
	A3($elm$core$Task$map2, $author$project$Utils$PosixZone, $elm$time$Time$now, $elm$time$Time$here));
var $author$project$Main$ScrollNoOp = {$: 'ScrollNoOp'};
		...
		switch (msg.$) {
			case 'GotEvents':
				...
				if (res.$ === 'Ok') {
					var vals = res.a;
					return _Utils_Tuple2(
						$author$project$Main$EventModelNoDate(vals),
						$author$project$Main$refreshDateTask);
				} else {
					var info = res.a;
					return _Utils_Tuple2(
						$author$project$Main$ErrorModel(
							$author$project$Main$errorToString(info)),
						$elm$core$Platform$Cmd$none);
				}
			case 'ReceiveDate':
				var posixzone = msg.a;
				var zone = posixzone.zone;
				var posix = posixzone.posix;
				var days = function () {
					switch (model.$) {
						case 'EventModelNoDate':
							var events = model.a;
							return events;
						case 'EventModelWithDate':
							var events = model.a;
							var date = model.b;
							return events;
						case 'LoadingModel':
...
var $author$project$Utils$jpFromDayOfWeek = function (day) {
	switch (day.$) {
		case 'Mon':
			return '月';
		case 'Tue':
...
var $author$project$Main$formatDate = F2(
	function (dt, withWeekday) {
		var yearStr = A3(
			$elm$core$String$padLeft,
			4,
			_Utils_chr('0'),
			$elm$core$String$fromInt(
				$PanagiotisGeorgiadis$elm_datetime$Calendar$getYear(dt)));
		var monthStr = A3(
			$elm$core$String$padLeft,
			2,
			_Utils_chr('0'),
			$elm$core$String$fromInt(
				$PanagiotisGeorgiadis$elm_datetime$Calendar$monthToInt(
					$PanagiotisGeorgiadis$elm_datetime$Calendar$getMonth(dt))));
		var dayStr = A3(
			$elm$core$String$padLeft,
			2,
			_Utils_chr('0'),
			$elm$core$String$fromInt(
				$PanagiotisGeorgiadis$elm_datetime$Calendar$getDay(dt)));
		var dayOfWeek = $author$project$Utils$jpFromDayOfWeek(
			$PanagiotisGeorgiadis$elm_datetime$Calendar$getWeekday(dt));
		return yearStr + ('/' + (monthStr + ('/' + (dayStr + function () {
			if (withWeekday) {
				return ' ' + ('(' + (dayOfWeek + ')'));
			} else {
				return '';
...
var $author$project$Main$renderBreak = F2(
	function (prev, next) {
		var interval = prev.to_time + (' -> ' + next.from_time);
		var intWithZeros = A2(
		...
		var timeParser = A2(
			...
			A2($elm$parser$Parser$ignorer, intWithZeros, $elm$parser$Parser$end));
		var to_result = A2($elm$parser$Parser$run, timeParser, next.from_time);
		var from_result = A2($elm$parser$Parser$run, timeParser, prev.to_time);
		var _v0 = _Utils_Tuple2(from_result, to_result);
		if ((_v0.a.$ === 'Ok') && (_v0.b.$ === 'Ok')) {
			...
			var from_h = _v2.a;
			var from_m = _v2.b;
			var diff_h_part = to_h - from_h;
			var diff_m_part = to_m - from_m;
			var diff_m_total = (diff_h_part * 60) + diff_m_part;
			var classes = _Utils_ap(
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('break')
					]),
				(diff_m_total > 15) ? _List_fromArray(
					[
						$elm$html$Html$Attributes$class('break-long')
					]) : _List_Nil);
			var diff_h = (diff_m_total / 60) | 0;
			var h_str = function () {
				if (!diff_h) {
					return '';
				} else {
					var hours = diff_h;
					return $elm$core$String$fromInt(diff_h) + 'h';
				}
			}();
			var diff_m = A2($elm$core$Basics$modBy, 60, diff_m_total);
			var m_str = function () {
				if (!diff_m) {
					return '';
				} else {
					var mins = diff_m;
					return $elm$core$String$fromInt(diff_m) + 'm ';
				}
			}();
			return ((diff_h >= 0) && (diff_m >= 0)) ? _List_fromArray(
				[
					A2(
					$elm$html$Html$span,
					classes,
					_List_fromArray(
						[
							$elm$html$Html$text(h_str + (m_str + 'break.'))
						]))
				]) : _List_Nil;
		} else {
			return _List_Nil;
...
var $author$project$Main$renderEvent = function (item) {
	var maybeflag = function () {
		var _v0 = item.flag;
		if (_v0) {
			return _List_fromArray(
				[
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text(item.from_time + (' - ' + item.to_time))
					])),
				A2(
				$elm$html$Html$span,
				_List_fromArray(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('@' + item.location)
					])),
				A2(
				$elm$html$Html$p,
				_List_Nil,
				_List_fromArray(
					[
						$elm$html$Html$text(item.info)
					]))
			]));
};
var $author$project$Main$renderBreakAndEvent = F2(
	function (prev, next) {
		return _Utils_ap(
			A2($author$project$Main$renderBreak, prev, next),
			_List_fromArray(
				[
					$author$project$Main$renderEvent(next)
				]));
	});
var $author$project$Main$renderBreaksAndEvents = F3(
	function (listSoFar, maybePrevious, eventsToRender) {
		renderBreaksAndEvents:
		while (true) {
			if (!eventsToRender.b) {
				return listSoFar;
			} else {
				var head = eventsToRender.a;
				var tail = eventsToRender.b;
				var rendered = function () {
					if (maybePrevious.$ === 'Nothing') {
						return _List_fromArray(
							[
								$author$project$Main$renderEvent(head)
							]);
					} else {
						var previous = maybePrevious.a;
						return A2($author$project$Main$renderBreakAndEvent, previous, head);
					}
				}();
				var $temp$listSoFar = _Utils_ap(listSoFar, rendered),
					$temp$maybePrevious = $elm$core$Maybe$Just(head),
					$temp$eventsToRender = tail;
				listSoFar = $temp$listSoFar;
				maybePrevious = $temp$maybePrevious;
				eventsToRender = $temp$eventsToRender;
				continue renderBreaksAndEvents;
			}
		}
	});
var $author$project$Main$renderEvents = function (events) {
	return A3($author$project$Main$renderBreaksAndEvents, _List_Nil, $elm$core$Maybe$Nothing, events);
};
var $author$project$Main$renderDay = F2(
	function (maybedatetime, day) {
		var parsedDate = $author$project$Utils$parseDate(day.date);
		var parsedDateStr = function () {
			if (parsedDate.$ === 'Just') {
				var datetime = parsedDate.a;
				return A2($author$project$Main$formatDate, datetime, true);
			} else {
				return day.date;
			}
		}();
		var numEvents = $elm$core$List$length(day.events);
		var isWeekend = function () {
			if (parsedDate.$ === 'Just') {
				var eventdate = parsedDate.a;
				var _v5 = $PanagiotisGeorgiadis$elm_datetime$Calendar$getWeekday(eventdate);
				switch (_v5.$) {
					case 'Sat':
						return true;
					case 'Sun':
		...
		var maybeWeekend = isWeekend ? _List_fromArray(
			[
				$elm$html$Html$Attributes$class('weekend')
			]) : _List_Nil;
		var events = $author$project$Main$renderEvents(day.events);
		var datestr = function () {
			if (maybedatetime.$ === 'Just') {
				var datetime = maybedatetime.a;
				return A2(
					$author$project$Main$formatDate,
					$PanagiotisGeorgiadis$elm_datetime$DateTime$getDate(datetime),
					false);
			} else {
				return '';
			}
		...
		var isToday = function () {
			if (maybedatetime.$ === 'Just') {
				...
				if (parsedDate.$ === 'Just') {
					...
					return _Utils_eq(day.date, datestr);
				}
			} else {
				return false;
			}
		}();
		var datestring = isToday ? 'today' : ('on ' + day.date);
		var summary = function () {
			var _v0 = day.events;
			if (!_v0.b) {
				return 'No events ' + (datestring + '!');
			} else {
				var some_events = _v0;
				return 'You have ' + ($elm$core$String$fromInt(numEvents) + (((numEvents === 1) ? ' event ' : ' events ') + (datestring + '.')));
			}
		}();
		var maybeToday = isToday ? _List_fromArray(
			[
				$elm$html$Html$Attributes$class('today'),
				$elm$html$Html$Attributes$id('today')
			]) : _List_Nil;
		var extraClasses = _Utils_ap(maybeToday, maybeWeekend);
		return A2(
			$elm$html$Html$div,
			_Utils_ap(
				_List_fromArray(
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text('Loading your plan...')
							])),
						A2(
						$elm$html$Html$p,
						_List_Nil,
						_List_fromArray(
							[
								$elm$html$Html$text('(waiting for loyola to give us your schedule)')
							]))
					]));
		case 'ErrorModel':
			var error = model.a;
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Ouch! something didn\'t work: ' + error)
					]));
		case 'EventModelNoDate':
			var days = model.a;
			return A2(
				...
				A2(
					$elm$core$List$map,
					$author$project$Main$renderDay($elm$core$Maybe$Nothing),
					days));
		default:
			var days = model.a;
			var time = model.b;
